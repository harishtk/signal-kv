...
import com.magikodes.practice.signalkv.util.SignalKvPreferences;
...
public final class DatabaseSecretProvider {
    ...
    private static @NonNull DatabaseSecret getOrCreate(@NonNull Context context) {
        String unencryptedSecret    = SignalKvPreferences.getDatabaseUnencryptedSecret(context);
        String encryptedSecret      = SignalKvPreferences.getDatabaseEncryptedSecret(context);
    ...
    private static @NonNull DatabaseSecret getUnencryptedDatabaseSecret(@NonNull Context context,
                                                                        @NonNull String unencryptedSecret) {
        try {
            ...
            if (Build.VERSION.SDK_INT < 23) {
                ...
                SignalKvPreferences.setDatabaseEncryptedSecret(context, encryptedSecret.serialize());
                SignalKvPreferences.setDatabaseUnencryptedSecret(context, null);
    ...
    private static @NonNull DatabaseSecret createAndStoreDatabaseSecret(@NonNull Context context) {
        ...
        if (Build.VERSION.SDK_INT >= 23) {
            ...
            SignalKvPreferences.setDatabaseUnencryptedSecret(context, databaseSecret.asString());
        }
